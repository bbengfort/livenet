# Scripts to handle LiveNet build and installation
...
.PHONY: all install build livenet deps test citest clean doc protobuf
...
install: build
	$(info $(GM) installing livenet and making configuration …)
	@ cp $(BUILD)/livenet /usr/local/bin/

# Build the various binaries and sources
build: protobuf livenet

# Build the livenet command and store in the build directory
livenet:
	$(info $(GM) compiling livenet executable …)
	@ $(GOBUILD) -o $(BUILD)/livenet ./cmd/livenet
...
doc:
	...
	@ open http://localhost:6060/pkg/github.com/bbengfort/livenet/
	@ $(GODOC) --http=:6060
